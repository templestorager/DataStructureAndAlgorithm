 ...
 *  SQL Schema
    ...
    Write an SQL query that reports for each install date, the number of players that installed the game on that day and 
    the day 1 retention.
...
SELECT 
    ...
    ROUND(SUM(
			CASE 
				WHEN DATEDIFF(install_date, t2.event_date) = -1 AND t2.games_played > 0 
				THEN 1 ELSE 0 END
	)  / COUNT(DISTINCT t1.player_id), 2) AS Day1_retention
FROM (
...
select
    a1.install_dt, count(a1.player_id) as installs,
    round(count(a2.player_id)/count(a1.player_id),2) as Day1_retention
from
    (select player_id, min(event_date) as install_dt
    from activity
    group by player_id) as a1 left join activity a2 on a1.player_id=a2.player_id and datediff(a1.install_dt,a2.event_date)=-1
group by
    a1.install_dt